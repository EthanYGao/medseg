...
def uvnet_2d_bn_roi_u2(dim_data, dim_label, num_class, phase='train'):
	net = caffe.NetSpec()
	############ d0 ############
	net.data = L.Input(input_param=dict(shape=dict(dim=dim_data)))
	if phase == "train":
	...
	############ get roi ############
	net.prob3 = L.Softmax(net.upscore3, axis=1)
	...
	############ u2 ############
	### a ### Second Deconvolution
	net.u2a_dconv, net.u2a_bn, net.u2a_scale, net.u2a_relu = deconv_bn_relu(net.u3f_relu, 256, pad=0, kernel_size=2, stride=2, propagate_down=False)
...
def uvnet_2d_bn_roi_u3(dim_data, dim_label, num_class, phase='train'):
	net = caffe.NetSpec()
	############ d0 ############
	net.data = L.Input(input_param=dict(shape=dict(dim=dim_data)))
	if phase == "train":
...
def make_uvnet(net, dim_data, dim_label, num_class, prototxt_train, prototxt_test):
	...
	'uvnet_2d_bn_roi_u2', 'uvnet_2d_bn_roi_u3']
	...
	if net == 'uvnet_2d_bn_roi_u2':
		use_global_stats = 0
		train_net = uvnet_2d_bn_roi_u2(dim_data, dim_label, num_class, phase='train')
		use_global_stats = 1
		test_net = uvnet_2d_bn_roi_u2(dim_data, dim_label, num_class, phase='test')
	if net == 'uvnet_2d_bn_roi_u3':
		use_global_stats = 0
		train_net = uvnet_2d_bn_roi_u3(dim_data, dim_label, num_class, phase='train')
		use_global_stats = 1
		test_net = uvnet_2d_bn_roi_u3(dim_data, dim_label, num_class, phase='test')
